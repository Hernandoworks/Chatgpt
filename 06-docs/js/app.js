...
export function getToken() {
  ...
  if (t) {
    sessionStorage.setItem("phoenix_pat", t);
    return t;
  }
  if (s) {
    document.getElementById("patInput").value = s;
    return s;
  }
  return "";
}
...
export function setTokenStatus() {
  ...
  const v = getToken();
  el.textContent = v ? "\u2705 Authenticated" : "\u26A0\uFE0F Enter PAT to trigger";
  el.style.color = v ? "var(--success)" : "var(--text-muted)";
}
  ...
  return fetch(`${BASE}${path}`, { ...opts, headers })
    .then((r) => {
      if (!r.ok && r.status === 401) {
        document.getElementById("patStatus").textContent = "Invalid token";
...
export function iconFor(conclusion) {
  ...
  if (conclusion === "failure" || conclusion === "cancelled")
    return '<span class="status-icon failure" aria-label="failure">&#x274C;</span>';
...
export function init() {
  ...
  patEl.addEventListener("input", () => {
    const v = patEl.value;
    if (v) sessionStorage.setItem("phoenix_pat", v);
    else sessionStorage.removeItem("phoenix_pat");
    setTokenStatus();
  });
